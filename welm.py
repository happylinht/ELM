...
class ELMRegressor(BaseELM, RegressorMixin):
    ...
    def _fit_regression(self, y ,c, w):
        """
        ...
        if (self.regressor is None):
            ...
            H = self.hidden_activations_
            mm = H.shape[1]
            b=np.ones([mm,1])
            cc = (b*weights).T
            H_weight = H*cc 
            temp = pinv2(H_weight)
            self.coefs_ = safe_sparse_dot(temp*(cc.T), y)            
...
#             except:
#                 mm = H.shape[1]
#                 matric_c = np.eye(mm)*0.00001
#                 temp1 = np.linalg.inv(temp+matric_c)
#             temp2 = np.dot(temp1,H.T)
#             self.coefs_ = np.dot(temp2,y)
...
#             mm = H.shape[1]
#             c2=c
#             matric_c = np.eye(mm)*c2
#             temp = safe_sparse_dot(H.T,W)
#             temp = safe_sparse_dot(temp,H)+matric_c          
#             H_plus = safe_sparse_dot(np.linalg.inv(temp),H.T)
#             self.coefs_ = safe_sparse_dot(H_plus, y)
...
# #             temp = H.T*cc
# # #             temp = safe_sparse_dot(temp,H)+matric_c   
# #             temp = safe_sparse_dot(temp,H)
# #             try:
# #                 H_plus = safe_sparse_dot(np.linalg.inv(temp),H.T)
# #             except:
# #                 H_plus = safe_sparse_dot(np.linalg.inv(temp+matric_c),H.T)
# #             self.coefs_ = safe_sparse_dot(H_plus, y)
        else:
    ...
    def fit(self, X, y, c, w):
        ...
        self._fit_regression(as_float_array(y, copy=True),c,w)
...
class ELMClassifier(BaseELM, ClassifierMixin):
    ...
    def fit(self, X, y,c,w ):
        ...
        self.elm_regressor_.fit(X, y_bin, c, w)
        return self
